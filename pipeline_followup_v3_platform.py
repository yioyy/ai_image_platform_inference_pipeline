...
def pipeline_followup_v3_platform(
    *,
    ...
    current_base = _select_current_study(studies, case_id=case_id)
...
def _build_followup_entry(
    *,
    ...
    entry["jump_series_instance_uid"] = _safe_str(prior_inst.get("dicom_series_instance_uid")) if prior_inst else ""
...
def _build_regress_placeholder(
    *,
    ...
    placeholder: Dict[str, Any] = {
        ...
        "mask_name": "",
        "diameter": "",
        "type": prior_inst.get("type", ""),
        "location": prior_inst.get("location", ""),
        "sub_location": prior_inst.get("sub_location"),
        "prob_max": prior_inst.get("prob_max", ""),
        "checked": prior_inst.get("checked", "1"),
        "is_ai": prior_inst.get("is_ai", "1"),
    ...
    placeholder["followup"] = [
        {
            ...
            "jump_series_instance_uid": _safe_str(prior_inst.get("dicom_series_instance_uid")),
...
def _resolve_case_dir(
    *,
    ...
    if current_case_id and current_case_date_key and study_date_key == current_case_date_key:
        return _ensure_case_dir(path_process, current_case_id)
...
def _select_current_study(items: List[StudyItem], *, case_id: Optional[str]) -> StudyItem:
    if case_id:
        for item in items:
            if item.case_id == case_id:
                return item
    latest = max(items, key=lambda item: item.study_date_key)
    duplicates = [item for item in items if item.study_date_key == latest.study_date_key]
    if len(duplicates) > 1:
...
def _build_sorted_mapping_one_based(
    *,
    ...
    sorted_list = [
        {"current_slice": z + 1, "followup_slice_list": [p + 1 for p in mapping[z]]}
        for z in range(current_slices)
    ]

    reverse_mapping: Dict[int, List[int]] = {i: [] for i in range(prior_slices)}
    for current_idx, prior_list in mapping.items():
        for prior_idx in prior_list:
            reverse_mapping[prior_idx].append(current_idx)

    reverse_sorted_list = [
        {"followup_slice": z + 1, "current_slice_list": [c + 1 for c in reverse_mapping[z]]}
        for z in range(prior_slices)
    ]
