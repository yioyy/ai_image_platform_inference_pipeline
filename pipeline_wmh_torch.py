# -*- coding: utf-8 -*-
"""
RADAX WMH AI Pipeline (Torch/nnUNet flow)

參考 `code/pipeline_wmh.sh` + `code/pipeline_wmh_torch.py` 的推論流程，
差異在最後「上傳」：
- 不再走 aiteam/orthanc 上傳
- 改為複製結果到 `upload_dir/<StudyInstanceUID>/wmh_model/<inference_id>/...`
...
def _generate_platform_json_and_dicomseg(*, patient_id: str, path_nnunet: str, path_code: str) -> str:
    ...
    from wmh_pipeline import WMHPipeline  # type: ignore
    ...
    from code_ai.pipeline.dicomseg.wmh import main as make_wmh_pred_json

    json_path = make_wmh_pred_json(_id=patient_id, path_root=Path(path_nnunet), group_id=56)
    return str(json_path)


def _generate_radax_prediction_json(*, patient_id: str, path_nnunet: str, model_id: str) -> str:
    from code_ai.pipeline.dicomseg.build_wmh import execute_rdx_platform_json
...
def pipeline_wmh(
    ...
    path_nnunet_model: str = "/home/david/pipeline/chuan/code/nnUNet/nnUNet_results/Dataset015_DeepLacune/nnUNetTrainer__nnUNetPlans__2d",
    path_processModel: str = "/home/david/pipeline/chuan/process/Deep_WMH/",
    ...
    *,
    ...
    from pre_wmh import preprocess_wmh_images  # type: ignore
    from post_wmh import postprocess_wmh_results  # type: ignore
    ...
    try:
        ...
        if gpumRate >= 0.6:
            ...
            study_uid = _try_get_study_instance_uid_from_dicom_dir(DicomDirs[0] if DicomDirs else "")
            _post_inference_complete(
                api_url=api_url,
                study_instance_uid=study_uid,
                model_name="wmh_model",
                inference_id=None,
                result="failed",
            )
            return
        ...
        T2FLAIR_file = _clean_path(Inputs[0])
        SynthSEG_file = _clean_path(Inputs[1]) if len(Inputs) > 1 else None

        if not DicomDirs:
            raise ValueError("DicomDirs 至少需要提供一個 T2FLAIR 目錄")
        path_DcmT2FLAIR = _clean_path(DicomDirs[0])
        ...
        if not preprocess_wmh_images(path_processID):
        ...
        cmd = [
            "python",
            os.path.join(path_code, "gpu_lacune_only_wmh.py"),
            "--path_code",
            path_code,
            "--path_process",
            path_processID,
        ...
        for src, dst in (
            (os.path.join(path_processID, "T2FLAIR.nii.gz"), os.path.join(path_nii_n, "T2FLAIR.nii.gz")),
            (os.path.join(path_processID, "SynthSEG.nii.gz"), os.path.join(path_nii_n, "SynthSEG.nii.gz")),
            (os.path.join(path_processID, "SynthSEG_Brain.nii.gz"), os.path.join(path_nii_n, "SynthSEG_Brain.nii.gz")),
        ...
        if not postprocess_wmh_results(
            patient_id=ID,
        ...
        with open(rdx_json_path, "r", encoding="utf-8") as f:
            payload = json.load(f)
        study_uid = str((payload.get("input_study_instance_uid") or [""])[0] or "") or _try_get_study_instance_uid_from_dicom_dir(path_DcmT2FLAIR)
        inference_id = str(payload.get("inference_id") or "")
        if not inference_id:
            inference_id = str(uuid.uuid4())

        study_dir = os.path.join(upload_dir, study_uid)
        infer_dir = os.path.join(study_dir, "wmh_model", inference_id)
        os.makedirs(infer_dir, exist_ok=True)

        shutil.copy(rdx_json_path, os.path.join(infer_dir, "prediction.json"))

        _copy_dicom_series_to_uid_folder(path_dcm_t2, infer_dir)
        ...
        logging.info("!!! %s post_wmh finish.", ID)
        print("end!!!")
        return

    except Exception as e:
        logging.error("Catch an exception.", exc_info=True)
        logging.error("pipeline_wmh failed: %s", str(e))
        study_uid = ""
        try:
            study_uid = _try_get_study_instance_uid_from_dicom_dir(DicomDirs[0] if DicomDirs else "")
        except Exception:
