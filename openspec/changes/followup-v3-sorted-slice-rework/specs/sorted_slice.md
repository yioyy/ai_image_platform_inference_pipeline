# Follow-up v3 sorted_slice 規格增量
...
### Requirement: sorted_slice 對照型態
原先 `followup_slice_list` 與 `current_slice_list` 的多對多設計改為一對一嚴格對照。

## REMOVED Requirements
### Requirement: 多對多映射
移除 `followup_slice_list` / `current_slice_list` 的多對多映射結構。
